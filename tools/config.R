...
# `make` runs its recipes through a POSIX shell, so the Cargo directories found
# by `tools/msrv.R` need their drive letters rewritten (`C:/x` -> `/c/x`) before
# they can be joined with `:` into a PATH.
if (is_windows) {
    cargo_bin <- sub(
        "^([A-Za-z]):/",
        "/\\L\\1/",
...
# used to replace @CARGO_PATH@
# `R CMD INSTALL` does not pass on the PATH assembled in this script, so the
# Cargo directories are handed to `make` separately as a fallback for sessions
# whose PATH predates the Rust installation.
.cargo_path <- paste(c("$(PATH)", cargo_bin), collapse = ":")
...
new_txt <- gsub("@CRAN_FLAGS@", .cran_flags, mv_txt) |>
    ...
    gsub("@CARGO_PATH@", .cargo_path, x = _)
